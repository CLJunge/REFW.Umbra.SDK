@echo off
::
:: remove_reframework_deps.bat  <assembly-name>
::
:: Removes all files from the REFramework managed dependencies folder
...
if "%ASSEMBLY_NAME%"=="" (
    ...
    echo [cleanup] Usage: remove_reframework_deps.bat "MyAssembly"
    exit /b 1
)
...
if not exist "!DEST!\" (
    echo [cleanup] Dependencies folder not found: !DEST!
    exit /b 0
)
