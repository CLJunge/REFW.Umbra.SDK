@echo off
::
:: deploy_reframework_plugin.bat  <source-glob>
::
:: Called by plugin project post-build events in Debug configuration.
:: Copies the built plugin files matching <source-glob> to the REFramework
:: managed plugin folder inside the configured game directory.
::
...
if "%SOURCE_PATTERN%"=="" (
    ...
    echo [deploy] Usage: deploy_reframework_plugin.bat "path\to\output\Name*"
    exit /b 1
)
...
if not exist "!DEST!\" (
    echo [deploy] REFramework plugin folder not found: !DEST!
