...
internal interface IConfigTransferFilePicker
{
    ...
    /// <param name="fallbackDirectory">The fallback directory used when <paramref name="currentPath"/> does not resolve to an existing directory.</param>
    ...
    bool TryPickImportPath(string? currentPath, string? fallbackDirectory, out string? selectedPath);
    ...
    /// <param name="selectedPath">Receives the selected path when the user completes the picker.</param>
    /// <returns><see langword="true"/> when a path was selected; otherwise, <see langword="false"/>.</returns>
    bool TryPickExportPath(string? currentPath, string? fallbackDirectory, out string? selectedPath);
}
