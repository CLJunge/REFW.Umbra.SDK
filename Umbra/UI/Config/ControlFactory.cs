...
internal static class ControlFactory
{
    ...
    private static readonly Dictionary<Type, Func<string, IParameter, LabelAlignmentGroup, INumericEditSink?, Action>> _defaultBuilders = new()
    {
    ...
    internal static (Action draw, IDisposable? resource) BuildDrawAction(
        IParameter parameter, string label, LabelAlignmentGroup alignGroup, INumericEditSink? numericEditSink = null)
    {
        ...
        if (_defaultBuilders.TryGetValue(parameter.ValueType, out var builder))
            return (builder(label, parameter, alignGroup, numericEditSink), null);
    ...
    private static Action BuildActionDraw(string label, IParameter parameter, LabelAlignmentGroup alignGroup, INumericEditSink? numericEditSink)
    {
        _ = alignGroup;
        _ = numericEditSink;
    ...
    private static Action BuildBoolDraw(string label, IParameter parameter, LabelAlignmentGroup alignGroup, INumericEditSink? numericEditSink)
    {
        _ = numericEditSink;
