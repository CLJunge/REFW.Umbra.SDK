...
- For logging in plugin code, use `Umbra.Logging.PluginLogger` — an **instance class** that forwards to `REFrameworkNET.API.LogInfo`, `API.LogWarning`, and `API.LogError`.
  ...
  - SDK internals should use the static `Logger` facade for raw, unconditional logging with no per-plugin prefix or minimum level. **Use the static `Logger` facade for plugin instance code only; runtime host/bootstrap/mutex infrastructure should not receive or pass `PluginLogger` instances.**
